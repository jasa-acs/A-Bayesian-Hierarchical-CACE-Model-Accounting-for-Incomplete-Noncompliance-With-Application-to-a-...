...
tabletext<-cbind(
  c("", "Study (Author, Year)", paste(data2$Author_year), "Overall RD (ITT)"),
  
  c("", "RD", paste(formatC(round(meandata, 3), format='f', digits=3 ), " (",
...
forestplot(tabletext, 
           ...
           txt_gp = own.f,
           mean = RD_for_each[, "mean"],
           lower = RD_for_each[, "lower"],
           upper = RD_for_each[, "upper"],
           new_page = TRUE, 
